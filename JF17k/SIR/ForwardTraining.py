...
__learning_rate = 5e-3
__max_epoch = 10  # describe how many iterations through the whole training data
__summaries_dir = 'ft_event_logs/'
__batch_size = 512
...
def forward_training(use_data_from_checkpoint):
    ...
    __test_duration = steps_per_epoch * 2
    __checkpoint_duration = __max_steps // 5

    print('steps per epoch: %s\ntest duration %s\ncheckpoint duration %s'
          % (steps_per_epoch, __test_duration, __checkpoint_duration))

    sess = tf.InteractiveSession()

    # set True as set_up_model's parameter, to represent forward training
    x, y_, y, embedding = PureNN.set_up_model(True)  # care about embedding's initialization in this stage
    # loss function
    ...
    if not use_data_from_checkpoint:
        ...
        if latest_ckpt and latest_ckpt.model_checkpoint_path:
            ...
            print('\nVariables restored from the latest checkpoint: %s' % latest_ckpt.model_checkpoint_path)
        else:
            print('\nfail to restore variables from the latest checkpoint')
            return

    original_embedding = sess.run(embedding)
    print('restored embedding:')
    ...
    if use_data_from_checkpoint:
        vars_added.append(embedding)

    tf.initialize_variables(vars_added).run()

    initialized_embedding = sess.run(embedding)
    print('\n\nadopted embedding')
    print(initialized_embedding)
    ...
    test_writer_0 = tf.train.SummaryWriter(__summaries_dir + 'test0')
    test_writer_1 = tf.train.SummaryWriter(__summaries_dir + 'test1')
    ...
    for i in range(1, fake_max_steps):
        ...
        # Record train set summaries, and train the model
        summary, _ = sess.run(
            [merged, train_step],
        ...
        if i % __checkpoint_duration == 0 or i == __max_steps:
            ...
            save_path = saver.save(
                ...
                global_step=i + iteration_base
            )
            print('\nVariables saved in %s' % save_path)
...
def main(_):
    ...
    print('use data from checkpoint: ', __FLAGS.current_run != 0)
    global __max_epoch
    __max_epoch = __FLAGS.max_epoch
    print('max epoch %s' % __max_epoch)
    forward_training(__FLAGS.current_run != 0)  # args: use_data_from_checkpoint
