...
launchThreads(){
	...
	do
		bash run.sh $splitDir/$split $dataDir/entity.txt $dataDir/Type/type.txt $dataDir/Type/entityType.txt $dataDir/Type/relationSchemaSimple.txt tripletFinal/$split.txt pairFinal/$split.txt > $midDir/$split &
	done
	wait
}
