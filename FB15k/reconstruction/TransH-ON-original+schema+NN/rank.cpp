...
class Evaluator{
	...
	void loadTest(char *testDataPath, char *trainDataPath, char *candidateInstancePath){
		...
		while (fscanf(testDataFile, "%s", str) != EOF){
		...
		int count = 0;
		while (fscanf(candidateInstanceFile, "%s", str) != EOF){
			int relIndex = relation2index[string(str)];
			...
			if (testPositive.count(relIndex) == 0){
				if (trainPositive.count(relIndex) == 0){
					...
					candidateInstance[index2relation[relIndex] + "\t" + index2entities[mainEntityIndex]].push_back(score);
				}
				else{
...
void Evaluator::evaluate(){
	ofstream result("./rankResult.txt");
